...
1. The `ZONEINFO` operating system environment variable, if it is set to point to a `zoneinfo.zip` file such as the one that is included in Go.
1. The operating system's timezone database, sometimes called `tzdata` or `zoneinfo`.

    {{site.data.alerts.callout_info}}
    CockroachDB on Windows does not use the operating system's timezone database.
    {{site.data.alerts.end}}

1. The timezone database that is embedded in Go.

In the following situations, [restart](node-shutdown.html) each node:

- If the operating system's timezone database is updated.
- If any node's timezone changes.
- If the definition of a timezone changes due to government or geopolitical decisions.
- If the `ZONEINFO` operating system environment variable is updated or if the file it points to is updated.
